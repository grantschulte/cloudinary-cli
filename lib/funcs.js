...
const fs = require('fs')
const { CLOUDINARY_CONFIG_FILE, CISCO_IMAGE_TYPE_MAP, CISCO_IMAGE_FOLDER_NAME } = require('../config')
...
exports.deleteCiscoImageSet = argv => {
  const prefix = `${CISCO_IMAGE_FOLDER_NAME}/${argv.id}`
  let images = [prefix]

  for (let prop in CISCO_IMAGE_TYPE_MAP) {
    const suffix = CISCO_IMAGE_TYPE_MAP[prop]
